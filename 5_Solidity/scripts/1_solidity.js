...
async function mappings() {
    ...
    const cName = "Lock4";
    // Change the contract address to your deployed contract address.
    const cAddress = "0x68B1D87F95878fE05B998F19b66F4baba5De1aed";
...
const checkBalanceBeforeAfter = async(signer, lockContract) => {
    ...
    let b1 = await signer.provider.getBalance(signer.getAddress());
    let tx = await lockContract.withdraw();
    await tx.wait();
    let b2 = await signer.provider.getBalance(signer.getAddress());
